...
var _ = Describe("ProtocolParameter", func() {
	Describe("Protocol", func() {
		DescribeTable("DestinationPort",
			...
			Entry("port 22", 22, false, "--dport 22"),
			Entry("port 22 - verbose", 22, true, "--destination-port 22"),
			Entry("port 7777", 7777, false, "--dport 7777"),
			Entry("port 7777 - verbose", 7777, true, "--destination-port 7777"),
		)
		...
		Describe("NotDestinationPortIf", func() {
			...
			DescribeTable("should build a valid flag, when predicate returns true",
				...
				Entry("port 22", 22, false, "! --dport 22"),
				Entry("port 22 - verbose", 22, true, "! --destination-port 22"),
				Entry("port 80", 80, false, "! --dport 80"),
				Entry("port 80 - verbose", 80, true, "! --destination-port 80"),
				Entry("port 8080", 8080, false, "! --dport 8080"),
				Entry("port 8080 - verbose", 8080, true, "! --destination-port 8080"),
				Entry("port 7777", 7777, false, "! --dport 7777"),
				Entry("port 7777 - verbose", 7777, true, "! --destination-port 7777"),
			)
		})
		...
		DescribeTable("Tcp",
			...
			Entry("no parameters", nil, false, "tcp"),
			Entry("no parameters - verbose", nil, true, "tcp"),
		...
		DescribeTable("Udp",
			...
			Entry("no parameters", nil, false, "udp"),
			Entry("no parameters - verbose", nil, true, "udp"),
			Entry("1 parameter (DestinationPort(53))",
				[]*TcpUdpParameter{DestinationPort(53)},
				false,
				"udp --dport 53",
			),
			...
			Entry("1 parameter (NotDestinationPort(53))",
				[]*TcpUdpParameter{NotDestinationPort(53)},
				false,
				"udp ! --dport 53",
			),
